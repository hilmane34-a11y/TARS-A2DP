...
static const char *tars_status_text = "TARS A2DP READY";
...
static size_t tars_pcm_write(const uint8_t *data, size_t len) {
    size_t written = 0;

    while (written < len && pcm_used < PCM_BUFFER_SIZE) {
        pcm_buffer[pcm_write_pos] = data[written];
...
static size_t tars_pcm_read(uint8_t *data, size_t len) {
    size_t read = 0;

    while (read < len && pcm_used > 0) {
        data[read] = pcm_buffer[pcm_read_pos];
...
static int32_t tars_a2dp_data_callback(uint8_t *data, int32_t len) {
    if (data == NULL || len <= 0) {
        return 0;
    }
    ...
    switch (event) {
        ...
        case ESP_A2D_CONNECTION_STATE_EVT:
            ...
            switch (param->conn_stat.state) {
                ...
                case ESP_A2D_CONNECTION_STATE_DISCONNECTED:
                    ...
                    tars_status_text = "A2DP DISCONNECTED";
                    break;

                case ESP_A2D_CONNECTION_STATE_CONNECTING:
                    tars_a2dp_connected = false;
                    tars_a2dp_connecting = true;
                    tars_status_text = "A2DP CONNECTING";
                    break;

                case ESP_A2D_CONNECTION_STATE_CONNECTED:
                    tars_a2dp_connected = true;
                    tars_a2dp_connecting = false;
                    tars_status_text = "A2DP CONNECTED";
                    break;

                case ESP_A2D_CONNECTION_STATE_DISCONNECTING:
                    tars_a2dp_connected = false;
                    tars_a2dp_connecting = false;
                    tars_status_text = "A2DP DISCONNECTING";
                    break;
        ...
        case ESP_A2D_AUDIO_STATE_EVT:
            ...
            if (
                ...
                tars_status_text = "A2DP AUDIO STREAMING";
            }
            else if (
                param->audio_stat.state ==
...
static mp_obj_t tars_a2dp_start(void) {
    ...
    if (ret != ESP_OK) {
        return mp_obj_new_str(
            ...
            strlen("ERROR: GAP CALLBACK FAILED")
        );
    }
    ...
    if (ret != ESP_OK) {
        return mp_obj_new_str(
            ...
            strlen("ERROR: A2DP CALLBACK FAILED")
        );
    }
    ...
    if (ret != ESP_OK) {
        return mp_obj_new_str(
            ...
            strlen("ERROR: AUDIO CALLBACK FAILED")
        );
    }
...
static mp_obj_t tars_a2dp_scan(void) {
    ...
    if (!tars_bt_started) {
        return mp_obj_new_str(
            ...
            strlen("ERROR: START BLUETOOTH FIRST")
        );
    }
    ...
    if (ret != ESP_OK) {
        return mp_obj_new_str(
            ...
            strlen("ERROR: BLUETOOTH SCAN FAILED")
        );
    }
    ...
    return mp_obj_new_str(
        ...
        strlen("TARS SCANNING FOR I7-TWS...")
    );
}
...
static mp_obj_t tars_a2dp_found(void) {
    ...
    if (!tars_device_found) {
        ...
        if (tars_scanning) {
            return mp_obj_new_str(
                "STILL SCANNING...",
                strlen("STILL SCANNING...")
            );
        }

        return mp_obj_new_str(
            "I7-TWS NOT FOUND",
            strlen("I7-TWS NOT FOUND")
        );
    }
...
static mp_obj_t tars_a2dp_connect(void) {
    ...
    if (ret != ESP_OK) {
        return mp_obj_new_str(
            ...
            strlen("ERROR: A2DP CONNECT FAILED")
        );
    }
    ...
    return mp_obj_new_str(
        ...
        strlen("TARS CONNECTING TO I7-TWS...")
    );
}
